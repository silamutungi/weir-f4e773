...
export default function Dashboard() {
  ...
  async function fetchSupplementalData() {
    try {
      ...
      const [earningsRes, licensesRes] = await Promise.all([
        supabase
          .from('earnings')
          .select('*')
          .eq('user_id', user.id)
  ...
  function statusColor(status: string) {
    switch (status) {
      ...
      case 'monetized': return 'var(--color-info)'
  ...
  const isPageLoading = loading && detectionsLoading
                    ...
                    {recentDetections.map((detection) => (
                      <div key={detection.id} className="flex items-center justify-between py-3 border-b last:border-b-0" style={{ borderColor: 'var(--color-border)' }}>
                        <div className="flex-1 min-w-0 mr-4">
                          <p className="text-sm font-medium truncate" style={{ color: 'var(--color-text)' }}>{detection.title}</p>
                          <p className="text-xs truncate mt-0.5" style={{ color: 'var(--color-text-secondary)' }}>{detection.platform} · {detection.url}</p>
